...
function notification($position) {
  if ($position == get_theme_mod('notification_position', 'top')) {
    if (get_theme_mod('notification_text') && !isset($_COOKIE['tofino-notification-closed'])) : ?>
      ...
      <div class="flex bg-green-800 text-white text-sm px-4 py-3 md:text-base alert notification <?php echo $position; ?>" role="alert"  id="tofino-notification">
        <div class="flex items-center w-full">
          <span class="w-full max-w-sm leading-tight sm:max-w-none md:text-center"><?php echo nl2br(get_theme_mod('notification_text')); ?></span>

          <button type="button" class="w-4 h-4 ml-auto js-close" data-dismiss="alert" aria-label="Close">
