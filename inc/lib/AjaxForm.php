...
class AjaxForm
{
  ...
  private function buildEmailBody(array $settings)
  {
    ...
    $message = str_replace('%email_logo%', get_template_directory_uri() . '/dist/img/creativedot-logo-2x.png', $message);
    $message = str_replace('%footer_logo%', get_template_directory_uri() . '/dist/img/creativedot-text-logo-2x.png', $message);
  ...
  {
    // nonce check
    // if (!$this->isValidNonce($this->post['security'])) {
