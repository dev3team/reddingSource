...
function clean_nav_classes($classes, $item) {
  ...
  if ($item->menu_item_parent == 0) {
    $new_classes[] = 'top-level-menu-item';
  }
...
// Cleanup Yoast Breadcrumb markup
function filter_wpseo_breadcrumb_separator($this_options_breadcrumbs_sep) {
  $this_options_breadcrumbs_sep = '<span class="breadcrumb-separator">' . svg(['file' => 'arrow-head']) . '</span>';
...
function filter_wpseo_breadcrumb_output_class($class) {
  $class = 'breadcrumb-wrapper';

  return $class;
}
add_filter('wpseo_breadcrumb_output_class', __NAMESPACE__ . '\\filter_wpseo_breadcrumb_output_class', 10, 1);


function filter_wpseo_breadcrumb_single_link_wrapper($element) {
  $element = "li";

  return $element;
}
add_filter('wpseo_breadcrumb_single_link_wrapper', __NAMESPACE__ . '\\filter_wpseo_breadcrumb_single_link_wrapper', 10, 1);


function filter_wpseo_breadcrumb_wrapper($wrapper){
	$wrapper = "ol";

	return $wrapper;
}
add_filter('wpseo_breadcrumb_output_wrapper', __NAMESPACE__ . '\\filter_wpseo_breadcrumb_wrapper', 10, 1);
